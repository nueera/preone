#!/usr/bin/env python3
"""
PreOne Token Migration — Pass 3
Fix teacher and parent portal pages.
Teacher uses --portal-* emerald tokens, parent uses --portal-* sky tokens.
But for now, we'll use the semantic foreground/muted/card tokens which auto-adapt.
"""
...
BASE = "/home/z/my-project/preone/src/app"

# Same replacements as Pass 1 but for teacher/parent
REPLACEMENTS = [
    # Combined light+dark text patterns
    ...
    (r'text-gray-300\s+dark:text-gray-600', 'text-muted-foreground'),

    # Single text patterns
    ...
    (r'divide-gray-300\b', 'divide-border'),
...
def main():
    files = []
    for portal in ['teacher', 'parent']:
        files.extend(glob.glob(os.path.join(BASE, portal, "**/*.tsx"), recursive=True))

    total_changes = 0
    files_changed = 0
    print(f"Pass 3: Scanning {len(files)} teacher/parent files...")
    print("=" * 60)
